...
@ReqMsgHandler(ModuleMsgIdConstant.Jcby.FIRE)
public class ReqFireHandler implements ReqMessageHandler {
  ...
  public ResMessage action(Player player, ReqMessage message) {
    ReqFireMsg msg = (ReqFireMsg) message;
    return jcbyMgr.fire(player, msg.getAngle());
  }
}
