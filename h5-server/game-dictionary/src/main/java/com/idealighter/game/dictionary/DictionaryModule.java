package com.idealighter.game.dictionary;
...
/**
 * 字典模块.更多商业源码，系统定制开发联系：https://t.me/usdtvps666
 *
 */
public class DictionaryModule extends PrivateModule {

  private static final Logger LOG = LoggerFactory.getLogger(DictionaryModule.class);

  /** 游戏数据字典数据库jdbc配置文件路径 . */
  // private static final String DICDB_JDBC_PATH =
  // "config/jdbc/dicdb.properties";
  ...
  /** 字段数据库mapper路径 . */
  private static final String MAPPER_PATH = "com.idealighter.game.dictionary.mapper";
  ...
  protected void configure() {
    install(new MyBatisModule() {
      ...
      protected void initialize() {
        ...
        mapperClasses = new ResolverUtil<Object>().find((type) -> type.isInterface(), MAPPER_PATH)
            .getClasses();
     ...
     */
    bind(SqlSessionFactory.class).annotatedWith(DicDb.class).to(SqlSessionFactory.class);
    expose(SqlSessionFactory.class).annotatedWith(DicDb.class);

    bind(SqlSession.class).annotatedWith(DicDb.class).toProvider(SqlSessionManagerProvider.class)
        .in(Scopes.SINGLETON);
    expose(SqlSession.class).annotatedWith(DicDb.class);

    bind(SqlSessionManager.class).annotatedWith(DicDb.class)
        .toProvider(SqlSessionManagerProvider.class).in(Scopes.SINGLETON);
    expose(SqlSessionManager.class).annotatedWith(DicDb.class);
